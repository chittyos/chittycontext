...
export async function displayStatus(contextManager) {
  ...
  services.forEach(service => {
    ...
    if (accountName) {
      ...
      if (accountDetails) {
        console.log(`${service.icon}  ${chalk.bold(service.name)}: ${chalk.green(accountName)}`);
        ...
        if (service.key === 'github' && accountDetails.token) {
          console.log(chalk.dim(`   Token: ${accountDetails.token.substring(0, 8)}...`));
        }
        if (service.key === 'google' && accountDetails.email) {
          console.log(chalk.dim(`   Email: ${accountDetails.email}`));
        }
      } else {
        console.log(`${service.icon}  ${chalk.bold(service.name)}: ${chalk.yellow(accountName)} ${chalk.red('(not configured)')}`));
      }
    } else {
      console.log(`${service.icon}  ${chalk.bold(service.name)}: ${chalk.dim('not set')}`);
    }
  });
...
export function displayAccountList(service, accounts, currentContext) {
  ...
  accounts.forEach(acc => {
    ...
    const name = acc.name === currentContext ? chalk.green.bold(acc.name) : acc.name;
