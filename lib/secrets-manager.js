/**
 * Secrets Manager - Manage chittysecrets vaults and distribute secrets
 */
...
export class SecretsManager {
  /**
   * Create a chittysecrets vault for a context
   */
  async createVault(contextName) {
    try {
      ...
      console.log(`✅ Created chittysecrets vault: ${vaultName}`);
  ...
  /**
   * Store a secret in chittysecrets
   */
  ...
  async distributeToNeon(projectId, secretName, secretValue) {
    try {
      ...
      // For now, we'll store in chittysecrets with neon reference
      ...
      console.log(
        `✅ Stored ${secretName} for Neon project: ${projectId} in chittysecrets`,
      );
      return {
        stored: true,
  ...
  /**
   * Sync secrets from chittysecrets to all configured services
   */
  ...
  async rotateSecret(vaultName, itemName, newValue, distributionConfig) {
    try {
      // Update secret in chittysecrets
      await execAsync(
